...
public class CareerRecommendationServiceImpl implements CareerRecommendationService {
    ...
    public void deleteRecommendation(Integer careerId) {
        CareerRecommendation recommendation = careerRecommendationRepository.findById(careerId) .orElseThrow(() -> new ResourceNotFoundException("Career Recommendation not found"));
        careerRecommendationRepository.delete(recommendation);
    }
    @Override
    public CareerRecommendation getRecommendationById(Integer careerId) { return careerRecommendationRepository.findById(careerId).orElseThrow(() -> new ResourceNotFoundException("Career Recommendation not found"));
    }
    @Override
