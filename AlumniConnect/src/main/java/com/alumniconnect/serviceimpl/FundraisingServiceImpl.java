...
public class FundraisingServiceImpl implements FundraisingService {
    ...
    public void deleteFundraising(Integer fundId) {
        Fundraising fundraising = fundraisingRepository.findById(fundId).orElseThrow(() -> new ResourceNotFoundException("Fundraising not found"));
        fundraisingRepository.delete(fundraising);
    }
    @Override
    public Fundraising getFundraisingById(Integer fundId) {
        return fundraisingRepository.findById(fundId) .orElseThrow(() -> new ResourceNotFoundException("Fundraising not found"));
    }
    @Override
